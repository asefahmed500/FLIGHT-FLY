...
import { CheckCircle2, ShieldCheck, Plane, Building2, Star, CreditCard, User, Download, ArrowRight, Compass, Package, Sticker, Ticket as TicketIcon, Phone, CalendarDays, Users, Globe2, FileBadge, Tag } from "lucide-react"
import type { LucideIcon } from "lucide-react"
import { useAuth } from "@/lib/auth-context"
import { createBooking, validatePromoCode } from "@/lib/app-data"
...
export function BookingModal({ isOpen, onClose, item }: BookingModalProps) {
  ...
  // Promo state (validated against /api/promos before submit).
  ...
  const applyPromo = async () => {
    ...
    if (!code || !user || promoChecking) return
    setPromoChecking(true)
    setPromoError("")
    try {
      const res = await validatePromoCode(user, code)
      if (res.valid && res.code && res.percentOff) {
        setPromo({ code: res.code, percentOff: res.percentOff })
      } else {
        setPromo(null)
        setPromoError(res.error || "That code is not valid or has expired.")
      }
    } catch {
      setPromo(null)
      setPromoError("Could not verify the code. Try again.")
    } finally {
  ...
  const discountedPrice = (() => {
    if (!promo || !item) return item?.price ?? ""
    const m = item.price.replace(/,/g, "").match(/([\d.]+)/)
    if (!m) return item.price
    const n = Number(m[1])
    if (!Number.isFinite(n)) return item.price
    const off = (n * promo.percentOff) / 100
    const final = Math.max(0, Math.round((n - off) * 100) / 100)
    return `$${Number.isInteger(final) ? final : final.toFixed(2)}`
  })()
  ...
  const onSubmit = async (data: BookingFormValues) => {
    ...
    try {
      const res = await createBooking(user, {
      ...
      pushToast({
        ...
        description: `Reference ${res.refId} is pending approval. Track it in your dashboard.`,
      })
    } catch (err) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleReset()}>
      <DialogContent className="sm:max-w-[560px] p-0 overflow-y-auto max-h-[92dvh] bg-white border-slate-200 shadow-2xl rounded-2xl">
        <div className="bg-[#111111] p-5 text-white relative">
        ...
        {step === "details" ? (
          <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200">
              <div>
                ...
                <p className="text-base font-semibold text-[#111111] mt-0.5">{item.title}</p>
                {item.rating && (
              ...
              <div className="text-right">
                ...
                {promo ? (
                  <>
                    <p className="text-xs font-medium text-slate-400 line-through">{item.price}</p>
                    <p className="text-xl font-semibold text-[#4F46E5]">{discountedPrice}</p>
                    <p className="text-[10px] font-semibold text-emerald-600">{promo.code} · -{promo.percentOff}%</p>
                  </>
                ) : (
                  <>
                    <p className="text-xl font-semibold text-[#4F46E5]">{item.price}</p>
                    {item.originalPrice && (
            ...
            <div className="space-y-1.5">
              <Label htmlFor="b-promo" className="text-xs font-medium text-slate-700">
              ...
              {promo ? (
                <div className="flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2.5">
                  <span className="flex items-center gap-2 text-xs font-semibold text-emerald-700">
                    <Tag className="h-3.5 w-3.5" /> {promo.code} applied — {promo.percentOff}% off
                  </span>
                  <button type="button" onClick={clearPromo} className="text-xs font-medium text-slate-500 hover:text-rose-600">
                    Remove
                  </button>
                </div>
              ) : (
                <div className="flex gap-2">
                  <Input
                    ...
                    placeholder="e.g. FLYGOLD45"
                    className="h-10 border-slate-200 font-mono text-xs uppercase"
                  />
                  <Button type="button" variant="outline" onClick={applyPromo} disabled={!promoInput.trim() || promoChecking} className="h-10 shrink-0 border-slate-200 text-xs font-medium">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div
                  ...
                  className={`border p-3.5 rounded-xl flex items-center justify-between cursor-pointer transition-all ${currentPaymentType === "card" ? "border-[#4F46E5] bg-blue-50/50 shadow-sm" : "border-slate-200 hover:border-slate-300"}`}
                >
                  <div className="flex items-center gap-2">
                    <CreditCard className="w-4 h-4 text-[#4F46E5]" />
                    <span className="text-xs font-medium text-[#111111]">Credit / Debit Card</span>
                  </div>
                ...
                <div
                  ...
                  className={`border p-3.5 rounded-xl flex items-center justify-between cursor-pointer transition-all ${currentPaymentType === "invoice" ? "border-[#4F46E5] bg-blue-50/50 shadow-sm" : "border-slate-200 hover:border-slate-300"}`}
                >
                  <div className="flex items-center gap-2">
          ...
          <div className="p-8 text-center space-y-4">
            ...
            <h3 className="text-2xl font-semibold text-[#111111] tracking-[-0.01em]">Reservation Submitted!</h3>
            ...
            {finalPrice && finalPrice !== item.price && (
              <p className="text-sm">
                <span className="text-slate-400 line-through mr-1.5">{item.price}</span>
                <span className="font-semibold text-[#4F46E5]">{finalPrice}</span>
                {promo && <span className="ml-1.5 text-[10px] font-semibold text-emerald-600">{promo.code} applied</span>}
              </p>
            )}
            <p className="text-xs text-slate-500">
              Track status in your dashboard — updates land in your notifications bell.
            </p>
            <div className="pt-4 flex items-center justify-center gap-3">
              <Button render={<Link href="/dashboard/bookings" />} onClick={handleReset} className="bg-[#111111] hover:bg-[#4F46E5] text-white px-6 h-11 font-medium">
                View My Booking
              </Button>
              <Button
                variant="outline"
                  ...
                  downloadETicket(
                    {
                      ...
                      discount: promo ? `-${promo.percentOff}%` : null,
                      finalPrice: finalPrice || item.price,
